...
-- v10 — AFFILIATE PROGRAM columns (full design note in section 10 below
-- and in supabase/migration-v10-affiliates.sql). Declared here because
-- later `language sql` functions reference them and SQL bodies are
-- validated at CREATE time.
--   referral_code     — the user-chosen affiliate code (unique, ci).
--   referred_by       — who recruited this account, set once at sign-up
--                       by handle_new_user(); pinned by profiles_guard.
--   affiliate_balance — commissions available to request as a payout.
--                       Credited by approve_deposit(), reserved by
--                       request_affiliate_payout(), refunded on reject.
--                       Like `balance`, protected by the 7b grant revoke
--                       (username-only UPDATE), NOT by the guard trigger.
...
create table if not exists public.markets (
  ...
  -- v25.17 — 'void' is the third verdict: the source cancelled the market
  -- and every stake was refunded (see void_feed_market below).
...
-- v25.17 — widen the domain on projects created before 'void' existed. The
-- table definition above only runs on a FRESH database, so this is what
-- actually reaches the live book (same statement as the v25.17 migration).
...
-- v25.40 — SHAREABLE BET SLIPS. One short token per fill, minted on demand by
-- the fill's owner (`create_bet_share`) and read back by anyone holding the
-- link (`public_bet_share`).
--
...
-- v25.40 — one token per fill, forever: re-sharing the same bet returns the
-- link that was already sent rather than minting a second one (and never
...
-- v25.40 — DELIBERATELY NO POLICIES BELOW for this one. RLS-on with zero
-- policies denies every direct read and write, so the table is reachable only
-- through the two security-definer functions. That is what keeps "list every
-- share token" off the table while `public_bet_share(token)` stays open to anon.
...
begin
  ...
  if tg_op = 'INSERT' then
    ...
    -- referral_code/affiliate_balance are deliberately NOT pinned — the
    -- v5 note above applies to them too (set_affiliate_code and
    -- request_affiliate_payout run with the CALLER's auth.uid()).
...
begin
  ...
  -- v10 — referral attribution: `ref_code` rides along in the same
  -- raw_user_meta_data. Best-effort by design — an unknown, own or
  -- banned-owner code is silently ignored; a typo in the referral field
...
--
-- v10: when the depositor was referred and this is their FIRST approved
-- deposit, the affiliate is credited 50% of it into affiliate_balance.
-- The affiliate_earnings UNIQUE(referred_user_id) + ON CONFLICT DO
-- NOTHING makes a double-credit impossible even under concurrent
-- approvals; the deposits count check keeps users whose first deposit
-- predates v10 from generating a commission on a LATER deposit.
-- (affiliate_earnings is created in section 10 — this function body is
-- only parsed at CALL time, so the forward reference is fine.)
...
-- v25.17 — THE THIRD VERDICT: the source VOIDED the market.
--
-- A feed market whose real-world event never happens does not resolve yes
-- or no. Polymarket calls it "50-50" and pays both sides exactly $0.50;
-- Kalshi reports `result: 'void'`. Neither is a Side, so settle_feed_market
-- could not be called for one, and the settle cron did the only thing it
-- could: freeze the market on the `closed` half and skip the payout, every
-- run, forever. Stakes stayed locked in the pool with no path out but an
-- admin ban. (Found on UFC Abu Dhabi, Turman vs Dulatov 2026-07-25 —
-- cancelled hours before the card, and the market's own rules say 50-50.)
--
-- The refund leg is ban_market_rpc()'s, minus the ban: every holder gets
-- their COST BASIS back — the dollars they actually paid — because a void
-- means the trade never really happened. Paying $0.50 a share instead would
-- hand a profit to whoever bought under 50c and a loss to whoever bought
-- above, on an event that was never contested. Cost basis is also bounded
-- by `collateral` by construction, so the solvency assert below cannot fire
-- on a healthy book.
--
-- Sibling migration: supabase/migration-v25.17-void-settlement.sql (also
-- widens markets_resolved_outcome_check to allow 'void' on existing DBs).
...
declare
  ...
  v_fees numeric;
begin
  -- Service role only, exactly like settle_feed_market(): this is the settle
  -- cron's verdict, not an admin button. An admin voiding by hand still has
  -- ban_market_rpc().
  if auth.uid() is not null then
    raise exception 'Service role only';
  end if;
  ...
  -- Cost basis back to every holder. Rounded per user, not per position, so
  -- someone who bought the same side five times is not paid five half-cent
  -- roundings.
  with refunds as (
  ...
  -- The pool is spent. `seed = 0` for the same reason payout_market() zeroes
  -- it: the exposure unwind above subtracts v_m.seed, so leaving it set would
  -- let a later path unwind the same seed a second time.
  update public.markets m
     set collateral       = 0,
         fees_accrued     = 0,
         yes_reserve      = 0,
...
--
-- v25.40 adds five BETTING numbers, and they are aggregates only:
--
--   bets_placed   — fills, all time.
--   bets_resolved — fills whose market settled WITH a winner. Voided markets
--                   are excluded from both sides of the ratio: the source
--                   cancelled the question and the stake was refunded, so
--                   counting one as a loss would misstate the user's record.
--   bets_won      — fills whose side won.
--   volume_traded — gross staked, all time.
--   best_multiple — largest payout-per-dollar on a winning fill
--                   (shares / stake). "My best call was 6.4x".
--
-- Still no market ids, no individual stakes, no open positions, no balance —
-- the boundary is unchanged, it just now has counts behind it. The
...
-- v25.40 — MINT (or re-return) A FILL'S SHARE TOKEN.
--
-- Callable by the signed-in owner of the fill and nobody else. Three ways to
-- name the bet, in order of precedence:
...
--
-- Always the CALLER's own row: `user_id = auth.uid()` is inside every branch,
-- so passing somebody else's trade id finds nothing.
...
--
-- Banned user or banned market -> zero rows, matching public_profile: a share
-- link is never a way around a ban.
...
-- v25.40 — bet_shares is definer-functions-only. RLS with no policies already
-- denies everything; this revokes the table grant too, so neither half of the
-- door is left open on its own.
revoke all on public.bet_shares from authenticated, anon;
...
-- v25.40 — minting a bet share needs an account; READING one deliberately does
-- not (a recipient without a login is the entire point of a share link).
-- `public_bet_share` and `public_profile` are the two anon doors, and their
-- select lists are the privacy boundary — see the notes on each.
...
-- ---------------------------------------------------------------------
-- 10. v10 — AFFILIATE PROGRAM
-- ---------------------------------------------------------------------
-- Existing projects can apply just this feature with the standalone
-- supabase/migration-v10-affiliates.sql (same statements + the v10
-- replacements of profiles_guard / handle_new_user / approve_deposit,
-- which in THIS file are edited in place above).
--
-- The program:
--   * Any signed-in user can claim a personal referral code (3-20 chars,
--     picked by the user, globally unique case-insensitively).
--   * New users may enter a code at sign-up; handle_new_user() stores
--     the attribution in profiles.referred_by (invalid codes are ignored
...
--
-- SECURITY — same posture as the rest of the schema: all money movement
-- via SECURITY DEFINER RPCs; the client has no write path to
-- affiliate_balance / referred_by / the new tables (authenticated's
-- UPDATE grant on profiles is username-only, and the new tables get no
-- write grants at all).
